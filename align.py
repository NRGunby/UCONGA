...
def align_to(mol_to_align, ref_mol, allow_inversion, center_id):
    ...
    '''
    ref_coords = numpy.array([i.coords for i in ref_mol.atoms if i.get_id() in center_id])
    to_align_center = numpy.array([i.coords for i in mol_to_align.atoms if i.get_id() in center_id])
    mol_to_align.translate(-1*to_align_center.mean(axis=0))
    coords_to_align = numpy.array([i.coords for i in mol_to_align.atoms if i.get_id() in center_id])
...
if __name__ == '__main__':
    ...
    f_help = 'File format to output results in: cml, xyz, gms (GAMESS input geometry), nw (nwchem input geometry), gauss (Gaussian input geometry)'
    ...
    out_names = [path.join(i[0], 'Aligned_{0}.{1}'.format(i[1], args.output_format)) for i in input_names]
